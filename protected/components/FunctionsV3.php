...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
        ...
        $top_menu[]=array('tag'=>"contact",
                ...
                'tag'=>"signup",'label'=>''.Yii::t("default","Login & Signup"),'itemOptions'=>array('class' => 'logbtn'),'linkOptions'=>array('class' => 'logbtna'),
    ...
    public static function merchantList($as_list=true,$with_select=false)
    {
    	...
    	if($idAdmin==1){
        	...
        	";
    	   	...
    	   	$stmt="SELECT * FROM
        	{{merchant}}
        	WHERE status in ('active') and socio=$idAdmin
        	ORDER BY restaurant_name ASC
    	    "; 
    	}
    ...
    public static function MerchantpushNewOrder($order_id='')
    {
    	...
    	if ( $res=Yii::app()->functions->getOrder($order_id)){
    		...
    		if ( $device=$db_ext->rst($stmt)){
		    	...
		    	$contenta=FunctionsV3::replaceTemplateTags($content,$pattern,$res);
