...
$action="<a href=\"$link\" >".Yii::t("default","Details")."</a>";
	...
	public function rptMerchantSalesSummaryReport()
	{
	    ...
	    $admin_info = (array)Yii::app()->functions->getAdminInfo(); 

        $idAdmin=$admin_info['admin_id'];
		...
		$slug=$this->data['slug']; 
		    ...
		    $stmt="
		SELECT SQL_CALC_FOUND_ROWS a.restaurant_name,a.socio,
		(
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	...
	    	foreach ($rows as $val) {    
	    	    if($idAdmin==$val['socio'] or $idAdmin==1){
	    		$feed_data['aaData'][]=array(	    		  
...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function merchantList()
	{
		...
		$aColumns = array(
		  ...
		  'package_id','socio','is_commission','status'
		);
		//activation_token
		$sWhere=''; $sOrder=''; $sLimit='';
		...
		if ( $res=$this->rst($stmt)){		
			...
			foreach ($res as $val) {	
					...
					$link_login='<br/><br/>
					<a target="_blank" href="'.$url_login.'"><div class="uk-badge">'.t("Ingresar").'</div></a>
					';
...
if (FunctionsV3::hasModuleAddon("merchantappv2")){    		
		...
		if (FunctionsV3::hasModuleAddon('driver')){
	    	...
	    	if ( $Validator->validate()){
	    		...
	    		if ( $db->insertData("{{driver}}",$params)){
    			    ...
    			    } else $this->msg=t("Su solicitud ha sido recibida, espere mientras validamos su solicitud");
	...
	{								    
		$aColumns = array(
		  'driver_id','username','first_name','municipio','phone',
		  'team_id','status'
		);
		$t=AjaxDataTables::AjaxData($aColumns);		
		if (isset($_GET['debug'])){
